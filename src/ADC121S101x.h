...
*/
const int CS_pin = D3;
const uint32_t SPICLCK = 1000000;
const uint32_t SPICLCKFAST = 20000000;
const uint8_t BITORDER = MSBFIRST; //LSBFIRST;
const uint32_t ON = PERIPHS_GPIO_BASEADDR + 4U;
const uint32_t OFF = PERIPHS_GPIO_BASEADDR + 8U;
...
int ADC12_single_conversion()
{
    ...
    Serial.printf("SPI adc reading = %c%c%c %c %c%c%c%c %c%c%c%c %c%c%c%c\t%u count \t%u mV\n", BYTE_TO_BINARY(rawADC >> 8), BYTE_TO_BINARY(rawADC), rawADC, /*ESP.getVcc()*/ V_REF * rawADC / (2 << 12));
    return rawADC;
}
